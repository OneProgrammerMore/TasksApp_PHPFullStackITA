.PHONY: up down full-restart full-restart-linux start composer-install composer-update ssh run-tests swagger-generate logs kill-containers help
...
full-restart-linux: ## Perform a full restart: stop containers, remove all data, and bring up again
	docker-compose down && docker system prune --all -f && docker-compose up --build -d
